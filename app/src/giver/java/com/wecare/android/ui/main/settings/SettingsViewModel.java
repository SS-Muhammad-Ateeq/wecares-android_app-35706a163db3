...
public class SettingsViewModel extends BaseViewModel<SettingsNavigator> {
    ...
    public void holdServices(String status) {
        setIsLoading(true);
        getCompositeDisposable().add(getDataManager().holdServices(status)
                .subscribeOn(getSchedulerProvider().io())
                .observeOn(getSchedulerProvider().ui())
                .subscribe(new Consumer<BaseResponse>() {
                    @Override
                    public void accept(BaseResponse baseResponse) throws Exception {
                        ...
                        if (baseResponse.isSuccess())
                            getNavigator().holdServicesSuccessfully(status.equals(AppConstants.PHP_TRUE));
                        else
                            getNavigator().holdServicesFailed(status.equals(AppConstants.PHP_TRUE));
                    }
                }, new Consumer<Throwable>() {
                    @Override
                    public void accept(Throwable throwable) throws Exception {
