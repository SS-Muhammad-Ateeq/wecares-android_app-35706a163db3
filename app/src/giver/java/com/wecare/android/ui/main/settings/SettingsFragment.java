...
public class SettingsFragment extends BaseFragment<FragmentSettingsBinding, SettingsViewModel> implements SettingsNavigator {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        binding.settingsProvidingServicesSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
            viewModel.holdServices(isChecked ? AppConstants.PHP_TRUE : AppConstants.PHP_FALSE);
            }
        });
    ...
    private void setUp() {
        ...
        binding.settingsProvidingServicesSwitch.setChecked(viewModel.getDataManager().getCurrentUserModel().getSettings().getEnableProvidingServices() == AppConstants.PHP_TRUE_RAW);
...
//        FileUtil.openWebPage(getActivity(), ServerKeys.KEY_ABOUT_US_URL);
        startActivity(WebViewActivity.newIntent(getActivity()).putExtra(AppConstants.ARGS_KEY_URL, WeCareUtils.getPageURL(AppConstants.CURRENT_LOCALE.equals(AppConstants.LANGUAGE_LOCALE_ENGLISH)?AppConstants.PAGE_ABOUT_US_ENGLISH:AppConstants.PAGE_ABOUT_US_ARABIC,viewModel.getDataManager().getPages())).putExtra(AppConstants.ARGS_KEY_TITLE, getString(R.string.about_the_app)));
...
//        FileUtil.openWebPage(getActivity(), ServerKeys.KEY_PRIVACY_POLICY_URL);
        startActivity(WebViewActivity.newIntent(getActivity()).putExtra(AppConstants.ARGS_KEY_URL, WeCareUtils.getPageURL(AppConstants.CURRENT_LOCALE.equals(AppConstants.LANGUAGE_LOCALE_ENGLISH)?AppConstants.PAGE_PRIVACY_POLICY_ENGLISH:AppConstants.PAGE_PRIVACY_POLICY_ARABIC,viewModel.getDataManager().getPages())).putExtra(AppConstants.ARGS_KEY_TITLE, getString(R.string.privacy_police)));
    }

    @Override
    public void termsConditions() {
//        FileUtil.openWebPage(getActivity(), ServerKeys.KEY_TERMS_CONDITIONS_URL);
        startActivity(WebViewActivity.newIntent(getActivity()).putExtra(AppConstants.ARGS_KEY_URL, WeCareUtils.getPageURL(AppConstants.CURRENT_LOCALE.equals(AppConstants.LANGUAGE_LOCALE_ENGLISH)?AppConstants.GIVER_TERMS_SERVICE_ENGLISH:AppConstants.GIVER_TERMS_SERVICE_ARABIC,viewModel.getDataManager().getPages())).putExtra(AppConstants.ARGS_KEY_TITLE, getString(R.string.terms_and_conditions)));
    ...
    public void onChangeLanguageClicked() {
        ...
        languageAdapter = new ArrayAdapter<>(getBaseActivity(),
                R.layout.simple_spinner_item, sortArr);
                ...
                switch (selectedIndex) {
                    case 0:
                        if (!viewModel.getDataManager().getAppLocale().equals(AppConstants.LANGUAGE_LOCALE_ARABIC)) {
                            WeCareUtils.setLocale(AppConstants.LANGUAGE_LOCALE_ARABIC, getActivity(), true);
                    ...
                    case 1:
                        if (!viewModel.getDataManager().getAppLocale().equals(AppConstants.LANGUAGE_LOCALE_ENGLISH)) {
                            WeCareUtils.setLocale(AppConstants.LANGUAGE_LOCALE_ENGLISH, getActivity(), true);
    ...
    public void rateUsClicked() {
        FileUtil.openWebPage(getActivity(), "https://play.google.com/store/apps/details?id=com.wecare.android");
    ...
    @Override
    public void holdServicesSuccessfully(boolean enabled) {
        viewModel.getDataManager().getCurrentUserModel().getSettings().setEnableProvidingServices(enabled ? AppConstants.PHP_TRUE_RAW : AppConstants.PHP_FALSE_RAW);

    }

    @Override
    public void holdServicesFailed(boolean enabled) {
